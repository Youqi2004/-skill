...
Default covers should be full-bleed product photography. Do not add a white or cream outer border unless the user explicitly asks for a framed poster treatment.
...
- Bottom micro text or tags are optional. If used, keep them as a light editorial metadata line, not pill chips, badges, or rounded labels.
- Avoid repeated brand micro text in the bottom-right corner. The official top logo is enough unless the user asks for an extra signature.
...
- Avoid chip-like tag groups, heavy rounded boxes, and decorative frames that make the cover feel like an app UI instead of premium product photography.
...
3. If using the default VOXU format, run `scripts/render_cover.py` for final official logo placement, headline, accent line, highlight, and export size. Its default output should be full-bleed, without outer borders, pill tags, or bottom-right brand repetition. Use `--logo-color` only when a non-white logo is needed for contrast or aesthetics.
