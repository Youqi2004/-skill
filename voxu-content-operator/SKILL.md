...
Return this outer structure by default, but do not force the body copy into a generic five-step template. When the user provides or mentions video content, separate the script used inside the video from the caption used when publishing the video:
...
Generate 5 Xiaohongshu title candidates, 5 Douyin title candidates, one recommended title for each platform, a video body/script, a publishing caption body, and 2-3 cover headline options.

The `视频正文` is for spoken narration, subtitles, or on-screen narrative. It should follow the user's described video scenes, use spoken rhythm, avoid hashtags, and keep product details in the order the video naturally reveals them.

The `发布视频时的简介正文` is the text shown beside or under the uploaded video. It should be shorter than the video script, summarize the hook and product value, include search-friendly keywords or hashtags when useful, and avoid simply repeating the full narration.
...
- Full-bleed product photography by default: no white outer border, no pill-shaped tag chips, and no repeated bottom-right brand text unless requested.
