...
def render(args: argparse.Namespace) -> None:
    ...
    w, h = size
    margin = int(min(w, h) * 0.052) if args.framed else 0
    ...
    inner_pad = int(min(frame_size) * 0.075)
    inner_x = margin + inner_pad
    inner_right = margin + frame_size[0] - inner_pad
    if args.ratio == "3x4":
        logo_y = margin + 76
        logo_center = w // 2
        title_x = inner_x
        title_y = margin + int(frame_size[1] * 0.735)
        max_title_w = int(frame_size[0] * 0.84)
        start_size = 84
    else:
        logo_y = margin + 70
        logo_center = inner_x + 176
        title_x = inner_x
        title_y = margin + int(frame_size[1] * 0.67)
        max_title_w = int(frame_size[0] * 0.47)
        start_size = 74
    ...
    if args.tags:
        tag_y = min(h - margin - 70, title_y + line_h * len(lines) + 32)
        tag_line = " / ".join([t.strip().upper() for t in re.split(r"[,，/|]", args.tags) if t.strip()][:3])
        tag_size = 24 if args.ratio == "3x4" else 20
        tag_font = font_regular(tag_size)
        while tag_line and text_size(draw, tag_line, tag_font)[0] > inner_right - title_x and tag_size > 16:
            tag_size -= 1
            tag_font = font_regular(tag_size)
        draw.text((title_x + 2, tag_y + 2), tag_line, font=tag_font, fill=(0, 0, 0, 125))
        draw.text((title_x, tag_y), tag_line, font=tag_font, fill=(255, 255, 255, 218))

    signature = args.signature.strip()
    if signature:
        brand_font = font_regular(22 if args.ratio == "3x4" else 20)
        micro_w, _ = text_size(draw, signature, brand_font)
        draw.text((inner_right - micro_w, h - margin - 56), signature, font=brand_font, fill=(255, 255, 255, 190))
...
def main() -> None:
    ...
    parser.add_argument("--tags", default="BODY STATUS,ACTIVITY,SLEEP", help="Comma-separated metadata words. Empty to hide.")
    parser.add_argument("--signature", default="", help="Optional bottom-right signature text. Empty by default.")
    parser.add_argument("--framed", action="store_true", help="Add the legacy cream outer frame and rounded photo mask.")
